 ...
 * the block's hash is initialized to zero;
 * the transaction list is initialized as an empty linked list to store
 ...
 *         during allocation or list creation
 */
		      ...
		      uint32_t data_len)
{
	...
	/* Initialize the transaction list as an empty linked list */
	block->transactions = llist_create(MT_SUPPORT_FALSE);

	if (!block->transactions)
	{
		fprintf(stderr, "block_create: Failed to create transaction list.\n");
		free(block);
